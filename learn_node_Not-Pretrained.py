...
if EMBEDDING_METHOD != 'Not-Pretrained':
    logger.info('Only for Not-Pretrained Model')
    logger.info('Exit Program')
    exit()

# Load data and train val test split
g_df = pd.read_csv(f'./processed/{SUBREDDIT}_structure.csv', index_col=0)
n_feat = torch.from_numpy(np.load(f'./processed/{SUBREDDIT}_node_feat_{WORD_EMBEDDING}.npy')).to(device)

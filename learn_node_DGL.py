...
n_feat = torch.cuda.FloatTensor(n_feat)
...
train_flag = (g_ts < train_time) & time_cut_flag
val_flag = (g_ts >= train_time) & (g_ts < test_time) & time_cut_flag
test_flag = (g_ts >= test_time) & time_cut_flag
...
if EMBEDDING_METHOD == 'dgl-GCN':
    model = GCN(n_feat.shape[1], n_feat.shape[1], 1)
elif EMBEDDING_METHOD == 'dgl-SAGE':
    model = SAGE(n_feat.shape[1], n_feat.shape[1], 1)
elif EMBEDDING_METHOD == 'dgl-GAT':
    model = GAT(n_feat.shape[1], n_feat.shape[1], 1)
...
logger.info('loading saved DGL model')

#select pre-trained model
pretrained_model = f'{EMBEDDING_METHOD}-{SUBREDDIT}-{WORD_EMBEDDING}-{BATCH_SIZE}-{LEARNING_RATE}.pth'
logger.info(spec[:])

model_path = f'./saved_models/pretrained_models/{pretrained_model}'
model.load_state_dict(torch.load(model_path))
logger.info(f'{EMBEDDING_METHOD} models loaded : {pretrained_model}')
...
def eval_epoch(src_l, dst_l, g_num_l, lr_model, data_type, num_layer=NODE_LAYER):
    ...
    with torch.no_grad():
        ...
        for i, k in enumerate(g_l):
            if k not in graph_label_map:
                continue
            src_l_cut = torch.Tensor(src_l[g_num_l == k])
            dst_l_cut = torch.Tensor(dst_l[g_num_l == k])
            temp_g = dgl.graph((src_l_cut.type(torch.int64), dst_l_cut.type(torch.int64)))
            block = dgl.to_block(temp_g).to(device)
            src_n_ids = block.srcdata[dgl.NID]
            input_features = torch.index_select(n_feat, 0, src_n_ids).to(device)

            label = graph_label_map[k]

            src_embed = model.out(block, input_features)
...
for epoch in tqdm(range(NUM_EPOCH)):
    ...
    for k in g_l:
        src_l_cut = torch.Tensor(train_src_l[train_g_num_l == k])
        dst_l_cut = torch.Tensor(train_dst_l[train_g_num_l == k])
        temp_g = dgl.graph((src_l_cut.type(torch.int64), dst_l_cut.type(torch.int64)))
        block = dgl.to_block(temp_g).to(device)
        src_n_ids = block.srcdata[dgl.NID]
        input_features = torch.index_select(n_feat, 0, src_n_ids).to(device)
    ...
    val_acc, val_auc, val_AP, val_recall, val_F1, val_loss = eval_epoch(val_src_l, val_dst_l, val_g_num_l, lr_model, 0)
...
test_acc, test_auc, test_AP, test_recall, test_F1, test_loss = eval_epoch(test_src_l, test_dst_l, test_g_num_l, lr_model, 1)

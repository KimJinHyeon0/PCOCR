...
class AttnModel(torch.nn.Module):
    ...
    def __init__(self, feat_dim, edge_dim, time_dim, 
        ...
        """
        args:
          input_dim: dim for the node features
        ...
        #self.edge_fc = torch.nn.Linear(self.edge_in_dim, self.input_dim, bias=False)
...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat,
        ...
        self.affinity_score = MergeLayer(self.feat_dim, self.feat_dim, self.feat_dim, 1) #torch.nn.Bilinear(self.input_dim, self.input_dim, 1, bias=True)
    ...
    def forward(self, src_idx_l, target_idx_l, cut_time_l, num_neighbors=20):
        ...
        affinity_score = self.affinity_score(src_embed, target_embed).squeeze(dim=-1)

        return affinity_score

    def contrast(self, src_idx_l, target_idx_l, cut_time_l, num_neighbors=20):
        src_embed = self.tem_conv(src_idx_l, cut_time_l, self.num_layers, num_neighbors)
        target_embed = self.tem_conv(target_idx_l, cut_time_l, self.num_layers, num_neighbors)
        prob = self.affinity_score(src_embed, target_embed).squeeze(dim=-1)
        return prob.sigmoid()

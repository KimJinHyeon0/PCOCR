...
if EMBEDDING_METHOD != 'ONES':
    logger.info('Only for ONES Model')
    logger.info('Exit Program')
    exit()

### Load data and train val test split
g_df = pd.read_csv('./processed/{}_structure.csv'.format(DATA))
e_feat = np.load('./processed/{}_np_ones_edge_feat.npy'.format(DATA))
if EMBEDDING_METHOD == 'ONES':
    n_feat = torch.from_numpy(np.load('./processed/{}_np_ones_edge_feat.npy'.format(DATA))).to(device)

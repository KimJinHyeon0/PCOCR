...
from learn_edge_SAGE import MeanAggregator, Encoder, SupervisedGraphSage
...
features = torch.nn.Embedding(n_feat.shape[0], n_feat.shape[1])
features.weight = torch.nn.Parameter(torch.FloatTensor(n_feat), requires_grad=False)
features.cuda()

agg1 = MeanAggregator(features, cuda=True)
enc1 = Encoder(features, n_feat.shape[1], n_feat.shape[1], adj_lists, agg1, gcn=False, cuda=True)
agg2 = MeanAggregator(lambda nodes: enc1(nodes).t(), cuda=True)
enc2 = Encoder(lambda nodes: enc1(nodes).t(), enc1.embed_dim, n_feat.shape[1], adj_lists, agg2,
...
test_acc, test_auc, test_AP, test_recall, test_F1, test_loss = eval_epoch(test_src_l, test_g_num_l, lr_model, 1)
